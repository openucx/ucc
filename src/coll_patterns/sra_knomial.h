...
static inline ucc_rank_t ucc_sra_kn_compute_step_radix(ucc_rank_t rank, ucc_rank_t size,
                                                       ucc_knomial_pattern_t *p)
{
    ...
    for(k = 1; k < p->radix; k++) {
        ...
        if (peer == UCC_KN_PEER_NULL) continue;
        step_radix += 1;
    }
    step_radix += 1;
    return step_radix;
...
static inline ucc_rank_t ucc_sra_kn_compute_seg_index(ucc_rank_t peer, ucc_rank_t kpow_num, ucc_knomial_pattern_t *p)
{
...
// segment size
static inline size_t ucc_sra_kn_compute_seg_size(size_t block_count, ucc_kn_radix_t radix, ucc_rank_t si)
{
    return block_count/radix + (si < (block_count % radix) ? 1 : 0);
}

// segment offset in exhange group of tree
static inline size_t ucc_sra_kn_compute_seg_offset(size_t block_count, ucc_kn_radix_t radix, ucc_rank_t si)
{
    return (block_count/radix)*si +
           ((si < (block_count % radix)) ? si :  (block_count % radix));
}

static inline size_t ucc_sra_kn_compute_block_count(size_t count, ucc_rank_t rank,
                                                 ucc_knomial_pattern_t *p)
{
...
static inline void ucc_sra_kn_get_offset_and_seglen(size_t count, size_t dt_size, ucc_rank_t rank,
                                       ...
                                       ptrdiff_t *offset, int *seglen)
{
    ...
    while(!ucc_knomial_pattern_loop_done(&p)) {
        ...
        my_seg_offset = ucc_sra_kn_compute_seg_offset(block_count, step_radix, my_si);
    ...
    if (step_radix) {
        my_seg_len    = ucc_sra_kn_compute_seg_size(block_count, step_radix, my_si);
    }
    if (offset) *offset = _offset;
    if (seglen) *seglen = my_seg_len;
}

static inline ptrdiff_t ucc_sra_kn_get_offset(size_t count, size_t dt_size, ucc_rank_t rank,
                                 ucc_rank_t size, ucc_kn_radix_t radix)
{
    ptrdiff_t offset;
