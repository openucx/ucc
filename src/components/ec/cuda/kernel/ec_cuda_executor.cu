...
#define TRY_REDUCE(_fn)                                                        \
    do {                                                                       \
        status = _fn(task, op, dt);                                            \
        if (status != UCC_ERR_NOT_SUPPORTED)                                   \
            return status;                                                     \
    } while (0)

    TRY_REDUCE(executor_reduce_int8);
    TRY_REDUCE(executor_reduce_int16);
    TRY_REDUCE(executor_reduce_int32);
    TRY_REDUCE(executor_reduce_int64);
    TRY_REDUCE(executor_reduce_uint8);
    TRY_REDUCE(executor_reduce_uint16);
    TRY_REDUCE(executor_reduce_uint32);
    TRY_REDUCE(executor_reduce_uint64);
    TRY_REDUCE(executor_reduce_float16);
    TRY_REDUCE(executor_reduce_float32);
    TRY_REDUCE(executor_reduce_float64);
    TRY_REDUCE(executor_reduce_bfloat16);
#undef TRY_REDUCE
    ...
    ucc_ec_cuda_executor_t *eee, unsigned num_threads, unsigned num_blocks)
{
    ...
    if (useCoopLaunch) {
        ...
        cudaLaunchCooperativeKernel(
            (void *)executor_kernel, dimGrid, dimBlock, kernelArgs, 0, stream);
    } else {
        executor_start<<<1, 1, 0, stream>>>(eee->dev_state, eee->dev_cidx);
        executor_kernel<<<num_blocks, num_threads, 0, stream>>>(
            eee, q_size, kernelUseCoop);
