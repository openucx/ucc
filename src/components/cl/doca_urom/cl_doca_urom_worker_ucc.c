 ...
 */
static doca_error_t notif_unpack(void *packed_notif, struct urom_worker_notify_ucc **ucc_notif)
{
 ...
 */
static void completion(struct doca_urom_worker_cmd_task *task, enum urom_worker_ucc_notify_type type)
{
	...
	task_data = (struct ucc_cl_doca_urom_task_data *)doca_urom_worker_cmd_task_get_user_data(task);
	if (task_data == NULL) {
...
error_exit:
	...
	case UROM_WORKER_NOTIFY_UCC_LIB_CREATE_COMPLETE:
		(task_data->lib_create)(result, task_data->cookie, ucc_notify->dpu_worker_id);
		break;
	case UROM_WORKER_NOTIFY_UCC_LIB_DESTROY_COMPLETE:
		(task_data->lib_destroy)(result, task_data->cookie, ucc_notify->dpu_worker_id);
		break;
	...
	case UROM_WORKER_NOTIFY_UCC_CONTEXT_DESTROY_COMPLETE:
		(task_data->ctx_destroy)(result, task_data->cookie, ucc_notify->dpu_worker_id);
		break;
	...
	case UROM_WORKER_NOTIFY_UCC_PASSIVE_DATA_CHANNEL_COMPLETE:
		...
		break;
	default:
		DOCA_LOG_ERR("Invalid UCC notification type %lu", ucc_notify->notify_type);
		break;
	}
...
doca_error_t ucc_cl_doca_urom_task_lib_create(struct doca_urom_worker *worker_ctx,
...
/*
 * UCC library destroy command completion callback function, user callback will be called inside the function
 *
 * @task [in]: UROM worker task
 * @task_user_data [in]: task user data
 * @ctx_user_data [in]: worker context user data
...
doca_error_t doca_urom_ucc_task_lib_destroy(struct doca_urom_worker *worker_ctx,
					    union doca_data cookie,
					    uint64_t dpu_worker_id,
...
/*
 * UCC context create command completion callback function, user callback will be called inside the function
 *
 * @task [in]: UROM worker task
 * @task_user_data [in]: task user data
 * @ctx_user_data [in]: worker context user data
...
doca_error_t ucc_cl_doca_urom_task_ctx_create(struct doca_urom_worker *worker_ctx,
					   union doca_data cookie,
					   uint64_t dpu_worker_id,
					   int64_t start,
					   int64_t *array,
...
/*
 * UCC context destroy command completion callback function, user callback will be called inside the function
 *
 * @task [in]: UROM worker task
 * @task_user_data [in]: task user data
 * @ctx_user_data [in]: worker context user data
...
doca_error_t ucc_cl_doca_urom_task_ctx_destroy(struct doca_urom_worker *worker_ctx,
					    union doca_data cookie,
					    uint64_t dpu_worker_id,
					    void *context,
...
doca_error_t ucc_cl_doca_urom_task_team_create(struct doca_urom_worker *worker_ctx,
					    union doca_data cookie,
					    uint64_t dpu_worker_id,
					    int64_t start,
					    int64_t stride,
...
/*
 * UCC collective command completion callback function, user callback will be called inside the function
 *
 * @task [in]: UROM worker task
 * @task_user_data [in]: task user data
 * @ctx_user_data [in]: worker context user data
...
doca_error_t ucc_cl_doca_urom_task_collective(struct doca_urom_worker *worker_ctx,
					   union doca_data cookie,
					   uint64_t dpu_worker_id,
					   void *coll_args,
					   void *team,
...
/*
 * UCC passive data channel command completion callback function, user callback will be called inside the function
 *
 * @task [in]: UROM worker task
 * @task_user_data [in]: task user data
 * @ctx_user_data [in]: worker context user data
...
doca_error_t ucc_cl_doca_urom_task_pd_channel(struct doca_urom_worker *worker_ctx,
					   union doca_data cookie,
					   uint64_t dpu_worker_id,
					   void *ucp_addr,
					   size_t addr_len,
					   ucc_cl_doca_urom_pd_channel_finished_cb cb)
{
	...
	/* Populate commands attributes */
	ucc_cmd->cmd_type = UROM_WORKER_CMD_UCC_CREATE_PASSIVE_DATA_CHANNEL;
	ucc_cmd->dpu_worker_id = dpu_worker_id;
