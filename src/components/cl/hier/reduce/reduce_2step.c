...
static ucc_status_t ucc_cl_hier_reduce_2step_finalize(ucc_coll_task_t *task)
{
    ...
    UCC_CL_HIER_PROFILE_REQUEST_EVENT(task, "cl_hier_reduce_2step_finalize", 0);
                                       ...
                                       ucc_schedule_t **sched_p, int n_frags)
{
    ...
    size_t                count     = (rank == root) ?
                                      args.args.dst.info.count :
                                      args.args.src.info.count;
    ucc_cl_hier_schedule_t *cl_schedule;
    ucc_schedule_t         *schedule;
    ucc_status_t            status;
    int                     n_tasks, i, first_task, root_on_local_node;

    root_on_local_node = ucc_team_ranks_on_same_node(root, rank, core_team);
    n_tasks            = 0;
    first_task         = 0;
    ...
    if (SBGP_ENABLED(cl_team, NODE_LEADERS)) {
        ...
        args.args.root = find_root_net_rank(ucc_team_rank_host_id(root, core_team),
                                            cl_team);
        status = ucc_coll_init(SCORE_MAP(cl_team, NODE_LEADERS),
                               &args, &tasks[n_tasks]);
        if (ucc_unlikely(UCC_OK != status)) {
            goto out;
        }
...
static ucc_status_t
ucc_cl_hier_reduce_2step_frag_init(ucc_base_coll_args_t *coll_args,
...
{
    ucc_cl_hier_team_t *cl_team = ucc_derived_of(schedule_p->super.super.team,
                                                 ucc_cl_hier_team_t);
    ...
    cl_schedule = ucc_derived_of(frag, ucc_cl_hier_schedule_t);
    scratch     = cl_schedule->scratch ? cl_schedule->scratch->addr : NULL;
    frag_count  = ucc_buffer_block_count(count, n_frags, frag_num);
    frag_offset = ucc_buffer_block_offset(count, n_frags, frag_num);
...
static ucc_status_t
ucc_cl_hier_reduce_2step_pipelined_start(ucc_coll_task_t *task)
{
...
static ucc_status_t
ucc_cl_hier_reduce_2step_pipelined_finalize(ucc_coll_task_t *task)
{
                         ...
                         ucc_coll_task_t **task)
{
    ...
    schedule->super.super.super.post           = ucc_cl_hier_reduce_2step_pipelined_start;
    schedule->super.super.super.finalize       = ucc_cl_hier_reduce_2step_pipelined_finalize;
