    ...
    for (i = 0; i < completions_num; i++) {
        ...
        if (wcs[i].wr_id == 0) {
            ...
            ucc_tl_mlx5_schedule_t *task = (ucc_tl_mlx5_schedule_t *)
                (uintptr_t)(wcs[i].wr_id & (~(uint64_t)0x1));
...
static ucc_status_t ucc_tl_mlx5_reg_fanin_start(ucc_coll_task_t *coll_task)
{
    ...
    task->alltoall.send_rcache_region_p =
        ucc_tl_mlx5_get_rcache_reg_data(send_ptr);
    ...
    if (reg_change_flag || (task->alltoall.send_rcache_region_p->mr->addr !=
                            a2a->previous_send_address[seq_index])) {
        flag |= UCC_MLX5_NEED_SEND_MKEY_UPDATE;
    }
    reg_change_flag = 0;
    if (UCC_OK !=
        ucc_rcache_get(ctx->rcache, (void *)TASK_ARGS(task).dst.info.buffer,
                       task->alltoall.msg_size * UCC_TL_TEAM_SIZE(team),
                       &reg_change_flag, &recv_ptr)) {
        tl_error(UCC_TASK_LIB(task), "Failed to register receive_bf memory");
        ucc_rcache_region_put(ctx->rcache,
                              task->alltoall.send_rcache_region_p->region);
        return UCC_ERR_NO_RESOURCE;
    }
    task->alltoall.recv_rcache_region_p =
        ucc_tl_mlx5_get_rcache_reg_data(recv_ptr);
    if (reg_change_flag || (task->alltoall.recv_rcache_region_p->mr->addr !=
...
ucc_status_t ucc_tl_mlx5_alltoall_finalize(ucc_coll_task_t *coll_task)
{
    ...
    a2a->previous_send_address[seq_index] =
        task->alltoall.send_rcache_region_p->mr->addr;
    a2a->previous_recv_address[seq_index] =
        task->alltoall.recv_rcache_region_p->mr->addr;
    ucc_rcache_region_put(ctx->rcache,
                          task->alltoall.send_rcache_region_p->region);
    ucc_rcache_region_put(ctx->rcache,
                          task->alltoall.recv_rcache_region_p->region);
