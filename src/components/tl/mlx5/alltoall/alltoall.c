...
static void build_rank_map(ucc_tl_mlx5_a2a_t *a2a, net_exchange_t *global_data,
                           size_t local_data_size)
{
    struct rank_data *data = ucc_malloc(sizeof(*data) * a2a->net.net_size);
    net_exchange_t *  d;
    int               i;
...
ucc_status_t ucc_tl_mlx5_a2a_init_start(ucc_tl_mlx5_team_t *team)
{
    ucc_tl_mlx5_context_t *ctx = UCC_TL_MLX5_TEAM_CTX(team);
    ucc_tl_mlx5_a2a_t *    a2a = NULL;
    ucc_sbgp_t *           node, *net;
    size_t                 storage_size;
    ...
    a2a->state = TL_MLX5_A2A_STATE_SHMID;
...
static ucc_status_t ucc_tl_mlx5_a2a_alloc_atomic(ucc_tl_mlx5_team_t *team)
{
    ucc_tl_mlx5_context_t *ctx = UCC_TL_MLX5_TEAM_CTX(team);
...
static ucc_status_t ucc_tl_mlx5_a2a_alloc_barrier(ucc_tl_mlx5_team_t *team)
{
    ucc_tl_mlx5_context_t *ctx = UCC_TL_MLX5_TEAM_CTX(team);
    ucc_tl_mlx5_a2a_t *    a2a = team->a2a;
    size_t                 size;
...
ucc_status_t ucc_tl_mlx5_a2a_init_progress(ucc_tl_mlx5_team_t *tl_team)
{
    ...
    case TL_MLX5_A2A_STATE_SHMID:
        ...
        for (i = 0; i < MAX_OUTSTANDING_OPS; i++) {
            ...
            op->my_ctrl = PTR_OFFSET(
                op->ctrl, node_rank * sizeof(ucc_tl_mlx5_a2a_ctrl_t));
        }
        ...
        if (a2a->node.asr_rank == node_rank) {
            ...
            status = ucc_tl_mlx5_a2a_alloc_barrier(team);
            if (UCC_OK != status) {
                goto err;
            }
            ...
            for (i = 0; i < MAX_OUTSTANDING_OPS; i++) {
                a2a->previous_msg_size[i] = 0;
            }

            status = ucc_tl_mlx5_a2a_init_umr(team->a2a, lib);
            if (status != UCC_OK) {
            ...
            status = ucc_tl_mlx5_init_mkeys(a2a, lib);
            if (status != UCC_OK) {
            ...
            a2a->state            = TL_MLX5_A2A_STATE_EXCHANGE;
            return UCC_INPROGRESS;
        }
    case TL_MLX5_A2A_STATE_EXCHANGE:
        if (a2a->node.asr_rank != node_rank) {
            break;
        }
        net_size        = a2a->net.net_size;
        ...
        build_rank_map(a2a, global_data, local_data_size);
        ...
        a2a->work_completion =
            ucc_malloc(sizeof(struct ibv_wc) * ucc_min(net_size, MIN_POLL_WC));
        if (!a2a->work_completion) {
            tl_error(lib, "failed to allocate %zd bytes for wc array",
                     sizeof(struct ibv_wc) * net_size);
            return UCC_ERR_NO_MEMORY;
        }
        /* memset(a2a->cq_completions, 0, sizeof(a2a->cq_completions)); */
...
void ucc_tl_mlx5_a2a_cleanup(ucc_tl_mlx5_team_t *team)
{
    ucc_tl_mlx5_a2a_t *a2a = team->a2a;
    ucc_base_lib_t *   lib = UCC_TL_TEAM_LIB(team);
    ucc_status_t       status;
    ...
    if (a2a->node.asr_rank == a2a->node.sbgp->group_rank) {
        ...
        ibv_dereg_mr(a2a->dummy_bf_mr);
        ucc_free(a2a->work_completion);
