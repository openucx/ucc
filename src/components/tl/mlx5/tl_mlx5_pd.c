...
void *do_sendmsg(void *ptr)
{
    ...
    *p                  = conn->fd;
//    tl_debug("sendmsg: %d, msg.msg_iovlen=%d", conn->fd, (int)msg.msg_iovlen);

    nbytes = sendmsg(conn->sock, &msg, 0);
    if (nbytes == -1) {
//        tl_error("Fail to send msg");
...
//    tl_debug("do_recvmsg: got len of %ld", iov[0].iov_len);
    ...
    if (nbytes == -1) {
//        tl_error("Failed to recv msg");
        return UCC_ERR_NO_MESSAGE;
    }

    p = (int *)CMSG_DATA(cmsghdr);
//    tl_debug("recvmsg: %d, nbytes=%d, handle1=%d", *p, (int)nbytes, handles[0]);
                                         ...
                                         int *sock_p, const char *sock_path)
{
    ...
    if (sock == -1) {
        tl_error(ctx->super.super.lib, "failed to create server socket errno %d",
                 errno);
        return UCC_ERR_NO_MESSAGE;
    }
    ...
    if (listen(sock, group_size) == -1) {
        tl_error(ctx->super.super.lib, "failed to listen to server socket errno %d", errno);
        return UCC_ERR_NO_MESSAGE;
    }
    *sock_p = sock;
    return UCC_OK;
                                      ...
                                      int is_asr, int asr_sock)
{
    ...
    if (!is_asr) {
        status = client_recv_data(&shared_ctx_fd, &shared_pd_handle, sock_path, lib);
        if (UCC_OK != status) {
            return status;
        }
