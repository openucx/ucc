                                                              ...
                                                              ucc_tl_mlx5_mcast_coll_req_t  *req)
{
    ...
    req_completed = (req->to_send == 0 && req->to_recv == 0);
    pp            = NULL;
...
static inline ucc_status_t ucc_tl_mlx5_mcast_do_bcast(ucc_tl_mlx5_mcast_coll_req_t *req)
{
    ...
    comm  = req->comm;
    wsize = comm->wsize;
    zcopy = req->proto != MCAST_PROTO_EAGER;
...
ucc_status_t ucc_tl_mlx5_mcast_test(ucc_tl_mlx5_mcast_coll_req_t* req)
{
    ...
    assert(req->comm->psn >= req->start_psn);

    status = ucc_tl_mlx5_mcast_do_bcast(req);
    if (UCC_OK == status) {
        assert(req->comm->ctx != NULL);
...
static inline int ucc_tl_mlx5_mcast_prepare_bcast(void* buf, int size, int root, void *mr,
                                                  ucc_tl_mlx5_mcast_coll_comm_t *comm,
                                                  ucc_tl_mlx5_mcast_coll_req_t  *req)
{
    ucc_status_t             status;
...
ucc_status_t ucc_tl_mlx5_mcast_coll_do_bcast(void* buf, int size, int root, void *mr,
                                             ucc_tl_mlx5_mcast_coll_comm_t *comm,
                                             ucc_tl_mlx5_mcast_coll_req_t **task_req_handle)
{
    ...
    tl_debug(comm->lib, "MCAST bcast start, buf %p, size %d, root %d, comm %d, "
                "comm_size %d, am_i_root %d comm->psn = %d \n",
                buf, size, root, comm->comm_id, comm->commsize, comm->rank ==
                root, comm->psn );

    req = ucc_malloc(sizeof(ucc_tl_mlx5_mcast_coll_req_t), "mcast_req");
    if (!req) {
...
ucc_status_t ucc_tl_mlx5_mcast_bcast_start(ucc_coll_task_t *coll_task)
{
    ...
    status = ucc_tl_mlx5_mcast_coll_do_bcast(buf, data_size, root, NULL, comm, &task->bcast_mcast.req_handle);
