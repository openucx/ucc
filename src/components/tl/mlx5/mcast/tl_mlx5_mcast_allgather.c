...
static inline void ucc_tl_mlx5_mcast_init_async_reliability_slots(ucc_tl_mlx5_mcast_coll_req_t *req)
{
    ...
    if (ONE_SIDED_ASYNCHRONOUS_PROTO == req->one_sided_reliability_scheme &&
        ...
        if (comm->cuda_mem_enabled) {
            ...
            if (ucc_tl_mlx5_mcast_memcpy(PTR_OFFSET(dest, ONE_SIDED_SLOTS_INFO_SIZE), UCC_MEMORY_TYPE_HOST,
                                         req->ptr, UCC_MEMORY_TYPE_CUDA, req->length, comm) != UCC_OK) {
                                                      ...
                                                      ucc_tl_mlx5_mcast_coll_req_t *req)
{
    ...
    if (req->length >= comm->max_per_packet && req->length % comm->max_per_packet != 0) {
        tl_debug(comm->lib, "Pipelined mcast allgather not supported: "
                "length (%ld) must be a multiple of max_per_packet (%d) for messages >= max_per_packet.",
                                              ...
                                              ucc_tl_mlx5_mcast_coll_req_t  *req)
{
    ...
    if (req->concurrency_level == 0) {
        ...
        tl_debug(comm->lib, "truly zero-copy allgather requires at least 2 multicast groups. "
                            ...
                            "Set UCC_TL_MLX5_MCAST_GROUP_COUNT=2 or higher to enable truly zero-copy.",
...
ucc_status_t ucc_tl_mlx5_mcast_allgather_init(ucc_tl_mlx5_task_t *task)
{
    ...
    if (comm->cuda_mem_enabled) {
        ...
        if (comm->allgather_comm.truly_zero_copy_allgather_enabled) {
            ...
            tl_debug(comm->lib, "CUDA message size %zu: using truly zero-copy (including small messages)", req->length);
        } else {
            req->proto = MCAST_PROTO_EAGER;
            tl_debug(comm->lib, "CUDA message size %zu: using staging with scratch buffer", req->length);
        }
    } else {
    ...
    if (req->proto == MCAST_PROTO_ZCOPY) {
       ...
       req->rreg = reg;
       req->mr   = reg->mr;
    } else if (req->proto == MCAST_PROTO_EAGER && comm->cuda_mem_enabled) {
        ...
        tl_debug(comm->lib, "allocated scratch buffer of size %zu for CUDA staging", scratch_size);

        /* Also register CUDA send buffer for zero-copy sending */
        status = ucc_tl_mlx5_mcast_mem_register(comm->ctx, req->ptr, req->length, &reg);
        if (UCC_OK != status) {
            tl_error(comm->lib, "CUDA sendbuf registration failed for zero-copy sending");
            goto failed;
        }
        req->rreg = reg;
        req->mr   = reg->mr;
        tl_debug(comm->lib, "registered CUDA send buffer for zero-copy sending");
    }
    ...
    if (comm->allgather_comm.truly_zero_copy_allgather_enabled) {
        ...
        if (UCC_OK != status) {
            tl_debug(comm->lib, "truly zero-copy allgather failed to prepare, falling back to staging: %s", ucc_status_string(status));
            goto failed;
        }
        req->progress = ucc_tl_mlx5_mcast_do_zero_copy_pipelined_allgather;
        tl_debug(comm->lib, "successfully enabled truly zero-copy allgather for message size %zu", req->length);
    } else {
        tl_debug(comm->lib, "truly zero-copy allgather not enabled, using staging protocol for message size %zu", req->length);
    }
