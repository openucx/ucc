                                                             ...
                                                             size_t send_offset)
{
    ...
    for (i = 0; i < num_packets; i++) {
        ...
        int use_zcopy = zcopy || (comm->cuda_mem_enabled && req->mr && req->mr != comm->pp_mr);
        ...
        if (!use_zcopy) {
            ucc_memory_type_t src_mem_type = comm->cuda_mem_enabled ? UCC_MEMORY_TYPE_CUDA : UCC_MEMORY_TYPE_HOST;
            ucc_memory_type_t dst_mem_type = UCC_MEMORY_TYPE_HOST; // staging buffer is always HOST
            status = ucc_tl_mlx5_mcast_memcpy((void*) pp->buf, dst_mem_type, req->ptr + offset, src_mem_type, length, comm);
            if (ucc_unlikely(status != UCC_OK)) {
        ...
        tl_trace(comm->lib, "mcast  post_send, psn %d, length %d, "
                "zcopy %d, use_zcopy %d, signaled %d qp->state %d qp->qp_num %d qp->pd %p "
                "mcast_group_index %d",
                 pp->psn, pp->length, zcopy, use_zcopy, swr[0].send_flags &
                 IBV_SEND_SIGNALED,
