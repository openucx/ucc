                                                           ...
                                                           int *completed)
{
    ...
    if (req->scratch_buf && req->scratch_packets_received > 0) {
        tl_debug(comm->lib, "reliability protocol starting - copying scratch buffer (%d packets received) to user buffer before RDMA reads",
        ...
        if (ucc_unlikely(status != UCC_OK)) {
            tl_error(comm->lib, "failed to copy scratch buffer to user buffer before reliability protocol");
            return status;
        }

        /* Mark scratch buffer as copied to avoid further operations on it */
        req->scratch_packets_received = -1;
        tl_debug(comm->lib, "successfully copied scratch buffer to user buffer before reliability protocol");
    }
        ...
        if (comm->allgather_comm.truly_zero_copy_allgather_enabled ||
            ...
            if (req->scratch_buf && comm->cuda_mem_enabled) {
                ...
                if (req->scratch_packets_received == (req->comm->commsize * req->num_packets)) {
                    ...
                    req->scratch_packets_received = -1;
                    tl_debug(comm->lib, "all packets received - copied scratch buffer to user buffer");
                }
            } else {
                ...
                ucc_memory_type_t dst_mem_type = comm->cuda_mem_enabled ? UCC_MEMORY_TYPE_CUDA : UCC_MEMORY_TYPE_HOST;
                ucc_memory_type_t src_mem_type = UCC_MEMORY_TYPE_HOST; // staging buffer is always HOST
                status = ucc_tl_mlx5_mcast_memcpy(dest, dst_mem_type, (void*) pp->buf, src_mem_type, pp->length, comm);
                if (ucc_unlikely(status != UCC_OK)) {
