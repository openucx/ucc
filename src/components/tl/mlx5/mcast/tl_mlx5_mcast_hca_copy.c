...
static ucc_status_t ucc_tl_mlx5_mcast_hca_copy_setup_resources(ucc_tl_mlx5_mcast_coll_comm_t *comm)
{
    ...
    if (!comm->hca_copy_cq) {
        ...
        cq_attr.cqe       = 16;  /* Small CQ for HCA copy only */
        cq_attr.comp_mask = 0;
    ...
    if (!comm->hca_copy_qp) {
        ...
        tl_debug(comm->lib, "HCA copy QP ready for loopback RDMA operations");
    }
...
static void ucc_tl_mlx5_mcast_hca_copy_completion(struct ibv_wc *wc, void *arg)
{
    ...
    if (wc->status != IBV_WC_SUCCESS) {
        tl_error(task->comm->lib, "HCA copy RDMA write failed with status %d", wc->status);
...
/* Post HCA copy operation - uses dedicated QP for true HCA-assisted copy with rcache optimization */
                                             ...
                                             ucc_tl_mlx5_mcast_hca_copy_task_t **copy_task)
{
    ...
    if (status != UCC_OK) {
        tl_error(comm->lib, "failed to register source buffer via rcache for HCA copy");
        ucc_free(task);
        return status;
    }
    ...
    if (status != UCC_OK) {
        tl_error(comm->lib, "failed to register destination buffer via rcache for HCA copy");
    ...
    *copy_task = task;
    tl_debug(comm->lib, "posted HCA RDMA write operation via rcache: %p -> %p, size %zu", src, dst, size);
    return UCC_OK;
}
...
ucc_status_t ucc_tl_mlx5_mcast_hca_copy_test(ucc_tl_mlx5_mcast_hca_copy_task_t *copy_task)
{
    ...
    /* Check completion */
    if (copy_task->completed) {
        tl_debug(copy_task->comm->lib, "HCA copy RDMA write completed successfully");
        return copy_task->status;
    }
                                      ...
                                      ucc_tl_mlx5_mcast_coll_comm_t *comm)
{
    ...
    if (comm->one_sided.hca_copy_enabled &&
        ...
        tl_debug(comm->lib, "using HCA-assisted copy with rcache for CUDA memory transfer (src: %s, dst: %s)",
        ...
        if (status == UCC_OK) {
            tl_debug(comm->lib, "HCA copy completed successfully");
            return UCC_OK;
        } else {
