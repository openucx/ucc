...
ucc_status_t ucc_tl_mlx5_allgather_mcast_init(ucc_base_coll_args_t *coll_args,
                                              ...
                                              ucc_coll_task_t     **task_h)
{
    ...
    if (UCC_COLL_ARGS_ACTIVE_SET(&coll_args->args)) {
        tl_trace(team->context->lib, "mcast allgather not supported for active sets");
    ...
    *task_h = &(task->super);
    ...
    tl_debug(UCC_TASK_LIB(task), "initialized mcast allgather coll task %p", task);
                                          ...
                                          ucc_coll_task_t     **task_h)
{
    ...
    *task_h = &(task->super);
    ...
    tl_debug(UCC_TASK_LIB(task), "initialized mcast bcast coll task %p", task);
                                   ...
                                   ucc_coll_task_t     **task_h)
{
    ...
    case UCC_COLL_TYPE_ALLGATHER:
        status = ucc_tl_mlx5_allgather_mcast_init(coll_args, team, task_h);
        break;
    default:
