...
static ucc_status_t
ucc_tl_sharp_mem_register(ucc_tl_sharp_context_t *ctx,
                          ucc_tl_sharp_team_t *team, void *addr,
                          size_t length, ucc_tl_sharp_reg_t **reg)
{
    ...
    if (ctx->sharp_context  != NULL) {
        sharp_ctx = ctx->sharp_context;
        rcache = ctx->rcache;
    } else {
...
static ucc_status_t
ucc_tl_sharp_mem_deregister(ucc_tl_sharp_context_t *ctx, ucc_tl_sharp_team_t *team,
        ...
        if (completed) {
            if (TASK_ARGS(task).coll_type == UCC_COLL_TYPE_ALLREDUCE) {
                if (!UCC_IS_INPLACE(TASK_ARGS(task))) {
                    ucc_tl_sharp_mem_deregister(TASK_CTX(task), TASK_TEAM(task),
...
ucc_status_t ucc_tl_sharp_bcast_start(ucc_coll_task_t *coll_task)
{
    ...
    ucc_tl_sharp_mem_register(TASK_CTX(task), team,
                              args->src.info.buffer, data_size,
                              &task->bcast.mem_h);
