...
static ucc_rank_t ucc_tl_shm_team_rank_to_group_rank(ucc_tl_shm_team_t *team, ucc_rank_t r) {
...
//    rank = UCC_TL_TEAM_RANK(self);
    ...
    self->tree_cache = (ucc_tl_shm_tree_cache_t *) ucc_malloc(max_trees *
                                  sizeof(ucc_tl_shm_tree_cache_keys_t) +
                                  max_trees * (sizeof(ucc_tl_shm_tree_t) *
                                  tree_size) + sizeof(size_t));
    if (!self->tree_cache) {
        return UCC_ERR_NO_MEMORY;
    }
    ...
    uint32_t cfg_ctrl_size = UCC_TL_SHM_TEAM_LIB(self)->cfg.ctrl_size;
    ...
    for (int i = 0; i < self->n_base_groups; i++) {
        ...
        for (int j=0; j < self->base_groups[i].group_size; j++) {
            team_rank = ucc_sbgp_rank2team(self->base_groups[i], j);
    ...
    self->ctrl_map = ucc_ep_map_from_array(rank_ctrl_offsets, size, size, 1); // ucc_ep_map_from_array64 create a 64 function?
