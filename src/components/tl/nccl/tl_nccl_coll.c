                                         ...
                                         cudaStream_t stream)
{
    ...
    if (status == UCC_INPROGRESS) {
        ucc_progress_enqueue(UCC_TL_CORE_CTX(TASK_TEAM(task))->pq, &task->super);
        return UCC_OK;
    }
...
ucc_status_t ucc_tl_nccl_alltoallv_start(ucc_coll_task_t *coll_task)
{
    ...
    for (peer = 0; peer < team->size; peer++) {
        count = ucc_coll_args_get_count(args,
                                        args->src.info_v.counts, peer);
        if (count != 0) {
            displ = ucc_coll_args_get_displacement(args,
                        args->src.info_v.displacements, peer);
        ...
        if (count != 0) {
            displ = ucc_coll_args_get_displacement(args,
                        args->dst.info_v.displacements, peer);
...
ucc_status_t ucc_tl_nccl_allreduce_init(ucc_tl_nccl_task_t *task)
{
    ...
    if (UCC_OK != ucc_nccl_check_dt_supported(task->super.args.src.info.datatype,
