...
__global__ void __launch_bounds__(UCC_TL_CUDA_MAX_NVLS_THREADS)
    allreduce_kernel_vec64(ucc_tl_cuda_nvls_control_t *mc_bar,
                           ucc_tl_cuda_nvls_control_t *uc_bar,
                           ...
                           uint32_t tsize)
{
    ...
    for (size_t idx = chunk_start + thread_offset; idx < chunk_end; idx += stride) {
        ...
        NvlsInt64Ops::st(val1, val2, base_u64 + idx);
    }
                                   ...
                                   uint32_t tsize, ucc_datatype_t datatype)
{
    ...
    case UCC_DT_INT32:
        assert(((uintptr_t)(mc_base_addr) % 8) == 0);
        allreduce_kernel_vec32<NvlsInt32Ops><<<sm_count, threads, 0, stream>>>(
            mc_bar, uc_bar, expected_blocks, launch_counter, base_u32, count_u32, rank, tsize);
        break;
    case UCC_DT_UINT32:
        assert(((uintptr_t)(mc_base_addr) % 8) == 0);
        allreduce_kernel_vec32<NvlsUint32Ops><<<sm_count, threads, 0, stream>>>(
            mc_bar, uc_bar, expected_blocks, launch_counter, base_u32, count_u32, rank, tsize);
        break;
    case UCC_DT_INT64:
        {
            ...
            allreduce_kernel_vec64<<<sm_count, threads, 0, stream>>>(
