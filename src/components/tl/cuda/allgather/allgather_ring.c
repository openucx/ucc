                                                      ...
                                                      uint32_t ring_id)
{
    ...
    while (step < (tsize * task->allgather_ring.num_frags)) {
        ...
        frag_offset = ucc_buffer_block_offset(ds, task->allgather_ring.num_frags,
                                            frag);
        frag_size = ucc_buffer_block_count(ds, task->allgather_ring.num_frags,
                                         frag);
        if (step % 2) {
...
ucc_status_t ucc_tl_cuda_allgather_ring_progress(ucc_coll_task_t *coll_task)
{
    ...
    case RING_STAGE_SYNC:
        ...
        task->allgather_ring.stage = RING_STAGE_SETUP;
    case RING_STAGE_SETUP:
        ...
        task->allgather_ring.stage = RING_STAGE_RING;
    case RING_STAGE_RING:
        ...
        task->allgather_ring.stage = RING_STAGE_BAR;
    default:
        ucc_assert(task->allgather_ring.stage == RING_STAGE_BAR);
        break;
    }
...
ucc_status_t ucc_tl_cuda_allgather_ring_start(ucc_coll_task_t *coll_task)
{
    ...
    task->allgather_ring.stage = RING_STAGE_SYNC;
