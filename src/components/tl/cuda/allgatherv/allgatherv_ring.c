                                                       ...
                                                       uint32_t ring_id)
{
    ...
    while (step < (tsize * task->allgatherv_ring.num_frags)) {
        ...
        block_offset = ucc_coll_args_get_displacement(args, args->dst.info_v.displacements, peer_block) *
                       ucc_dt_size(dt);
        frag_offset = ucc_ring_block_offset(block_size, task->allgatherv_ring.num_frags,
                                            frag);
        frag_size = ucc_ring_block_count(block_size, task->allgatherv_ring.num_frags,
                                         frag);
        if (step % 2) {
...
ucc_status_t ucc_tl_cuda_allgatherv_ring_progress(ucc_coll_task_t *coll_task)
{
    ...
    case ALLGATHER_RING_STAGE_SYNC:
        ...
        task->allgatherv_ring.stage = ALLGATHER_RING_STAGE_RING;
        st = ucc_tl_cuda_allgatherv_ring_setup_start(task);
        if (st != UCC_OK) {
            task->super.super.status = st;
            return task->super.super.status;
        }
        task->allgatherv_ring.stage = ALLGATHER_RING_STAGE_SETUP;
    ...
    case ALLGATHER_RING_STAGE_RING:
        ...
        task->allgatherv_ring.stage = ALLGATHER_RING_STAGE_BAR;
    default:
        ucc_assert(task->allgatherv_ring.stage == ALLGATHER_RING_STAGE_BAR);
        break;
    }
...
ucc_status_t ucc_tl_cuda_allgatherv_ring_start(ucc_coll_task_t *coll_task)
{
    ...
    task->allgatherv_ring.stage = ALLGATHER_RING_STAGE_SYNC;
