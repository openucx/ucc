...
/* Fixed packed payload layout for the CUDA TL memory handle:
 * [cudaIpcMemHandle_t][base_address][length][offset] */
#define UCC_TL_CUDA_MEMH_PACKED_SIZE                                          \
    (sizeof(cudaIpcMemHandle_t) + sizeof(void *) + sizeof(size_t) +           \
     sizeof(ptrdiff_t))
    ...
    if (mode == UCC_MEM_MAP_MODE_EXPORT) {
        ...
        if (memh->num_tls > 0) {
            ...
            for (i = 0; i < memh->num_tls && !found; i++) {
                if (offset + hdr_len > memh->pack_buffer_len) {
                    tl_error(ctx->super.super.lib,
                             "imported memh record %d header exceeds "
                             "pack_buffer bounds", i);
                    ucc_free(m_data);
                    tl_h->tl_data = NULL;
                ...
                if (offset + hdr_len + *packed_size > memh->pack_buffer_len) {
                    tl_error(ctx->super.super.lib,
                             "imported memh record %d payload exceeds "
                             "pack_buffer bounds", i);
                    ucc_free(m_data);
                    tl_h->tl_data = NULL;
                    return UCC_ERR_INVALID_PARAM;
                ...
                if (strncmp(tl_name, "cuda", UCC_MEM_MAP_TL_NAME_LEN) == 0) {
                    ...
                    if (*packed_size < UCC_TL_CUDA_MEMH_PACKED_SIZE) {
                        tl_error(ctx->super.super.lib,
                                 "malformed CUDA TL record: packed_size %zu is "
                                 "smaller than required %zu, skipping",
                                 *packed_size, UCC_TL_CUDA_MEMH_PACKED_SIZE);
                        ucc_free(m_data);
                        tl_h->tl_data = NULL;
                        return UCC_ERR_INVALID_PARAM;
                    }
    ...
    ucc_mem_map_tl_t *tl_h, void **pack_buffer)
{
    ...
    if (mode != UCC_MEM_MAP_MODE_EXPORT &&
        mode != UCC_MEM_MAP_MODE_EXPORT_OFFLOAD) {
        tl_debug(
            ctx->super.super.lib,
     ...
     */
    tl_h->packed_size = UCC_TL_CUDA_MEMH_PACKED_SIZE;
