...
ucc_status_t ucc_tl_ucp_allgatherv_ring_progress(ucc_coll_task_t *coll_task)
{
    ...
    while (task->send_posted < gsize) {
        ...
        data_displ = ucc_coll_args_get_displacement(args,
                        args->dst.info_v.displacements, send_idx) *
                        rdt_size;
        data_size = ucc_coll_args_get_count(args,
                        args->dst.info_v.counts, send_idx) * rdt_size;
        ...
        data_displ = ucc_coll_args_get_displacement(args,
                        args->dst.info_v.displacements, recv_idx) *
                        rdt_size;
        data_size = ucc_coll_args_get_count(args,
                        args->dst.info_v.counts, recv_idx) * rdt_size;
...
ucc_status_t ucc_tl_ucp_allgatherv_ring_start(ucc_coll_task_t *coll_task)
{
    ...
    if (!UCC_IS_INPLACE(coll_task->args)) {
        ...
        data_displ = ucc_coll_args_get_displacement(
                        &coll_task->args, coll_task->args.dst.info_v.displacements,
                        grank) * rdt_size;
        data_size  = ucc_coll_args_get_count(
                        &coll_task->args, coll_task->args.dst.info_v.counts,
                        grank) *rdt_size;
        UCPCHECK_GOTO(ucc_tl_ucp_recv_nb((void *)rbuf + data_displ, data_size,
