...
ucc_tl_ucp_allreduce_ring_start(ucc_coll_task_t *task)
{
    ...
    if (UCC_IS_INPLACE(*args)) {
         ...
         * (e.g. CUDA managed memory) the cost is non-trivial, but it avoids
         * threading src through both sub-collectives' ring logic while still
...
ucc_tl_ucp_allreduce_ring_finalize(ucc_coll_task_t *task)
{
    ...
    UCC_TL_UCP_PROFILE_REQUEST_EVENT(task, "ucp_allreduce_ring_done", 0);
