                      ...
                      ucc_memory_type_t memt)
{
    ...
    /* Copy data from scratch buffer to the final destination buffer */
    if (recv_n > 0) {
        void *final_buf = PTR_OFFSET(rbuf, recv_off);

        st = ucc_mc_memcpy(final_buf, task->allreduce_ring.scratch, recv_n, memt, memt);
        if (ucc_unlikely(st != UCC_OK)) {
            tl_error(UCC_TASK_LIB(task), "Failed to copy data from scratch to destination: %s",
                     ucc_status_string(st));
            task->super.status = st;
            return 0;
        }
...
ucc_status_t ucc_tl_ucp_allreduce_ring_init_common(ucc_tl_ucp_task_t *task)
{
    ...
    chunk_size = task->allreduce_ring.chunk_size = elems_per_chunk * dt_size;
