...
const char *ucc_tl_ucp_allreduce_alg_str[] = {
    [UCC_TL_UCP_ALLREDUCE_ALG_KNOMIAL]     = "knomial",
    [UCC_TL_UCP_ALLREDUCE_ALG_SRA_KNOMIAL] = "sra_knomial",
    [UCC_TL_UCP_ALLREDUCE_ALG_LAST]        = NULL
};

#define CHECK_USERDEFINED_OP(_args, _team) do {                              \
...
#define CHECK_INPLACE(_args, _team) do {                                      \
        if (!UCC_IS_INPLACE(_args) && (_args.src.info.mem_type != \
