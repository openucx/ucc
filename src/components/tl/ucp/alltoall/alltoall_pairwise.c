...
ucc_status_t ucc_tl_ucp_alltoall_pairwise_progress(ucc_coll_task_t *coll_task)
{
    ...
    chunk = UCC_TL_UCP_TEAM_CTX(team)->cfg.alltoall_pairwise_chunk;
    nreqs = (chunk > gsize || chunk == 0) ? gsize: chunk;
    data_size = (size_t)task->args.buffer_info.src_counts *
                ucc_dt_size(task->args.buffer_info.src_datatype);
    while ((task->send_posted < gsize || task->recv_posted < gsize) &&
        ...
        while ((task->recv_posted < gsize) &&
            ...
            ucc_tl_ucp_recv_nb((void*)(rbuf + peer * data_size), data_size,
                               UCC_MEMORY_TYPE_UNKNOWN, peer, team, task);
            polls = 0;
        }
