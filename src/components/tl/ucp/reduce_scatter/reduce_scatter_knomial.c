...
#define GET_COUNT(_args, _size)                                                \
    ({                                                                         \
        ...
        case UCC_COLL_TYPE_REDUCE_SCATTERV:                                    \
            _count = ucc_coll_args_get_total_count(                            \
                (_args), (_args)->dst.info_v.counts, (_size));                 \
        default:                                                               \
            break;                                                             \
        }                                                                      \
        _count;                                                                \
...
#define GET_DATA_BUF(_args)                                                    \
    ({                                                                         \
        ...
        case UCC_COLL_TYPE_ALLREDUCE:                                          \
        case UCC_COLL_TYPE_REDUCE_SCATTER:                                     \
            _buf = UCC_IS_INPLACE(*(_args)) ? (_args)->dst.info.buffer         \
                                            : (_args)->src.info.buffer;        \
            break;                                                             \
        case UCC_COLL_TYPE_REDUCE_SCATTERV:                                    \
            _buf = UCC_IS_INPLACE(*(_args)) ? (_args)->dst.info_v.buffer       \
                                            : (_args)->src.info_v.buffer;      \
        default:                                                               \
            break;                                                             \
        }                                                                      \
        _buf;                                                                  \
...
static inline ucc_status_t reduce_blocks(ucc_tl_ucp_task_t *task)
{
    ...
    if (n_recv > 0) {
        ucc_kn_rs_pattern_peer_seg(rank, p, &local_seg_count,
                                   &local_seg_offset);
        local_block = PTR_OFFSET(
            task->reduce_scatter_kn
                .loop_sbuf[!ucc_knomial_pattern_loop_first_iteration(p)],
            local_seg_offset * dt_size);
        recv_blocks =
            task->reduce_scatter_kn
                .loop_rbuf[!ucc_knomial_pattern_loop_first_iteration(p)];
        ...
        return ucc_tl_ucp_reduce_multi(
            local_block, recv_blocks, reduce_target, n_recv, local_seg_count,
...
ucc_tl_ucp_reduce_scatter_knomial_progress(ucc_coll_task_t *coll_task)
{
    ...
    if (KN_NODE_EXTRA == node_type) {
        ...
        UCPCHECK_GOTO(ucc_tl_ucp_send_nb(data_buf, count * dt_size, mem_type,
                                         peer, team, task),
                      task, out);
        if (p->type != KN_PATTERN_REDUCE_SCATTERX) {
            local_count = args->coll_type == UCC_COLL_TYPE_REDUCE_SCATTERV
                              ? ucc_coll_args_get_count(
                                    args, args->dst.info_v.counts, rank)
                              : count / size;
            UCPCHECK_GOTO(ucc_tl_ucp_recv_nb(dst, local_count * dt_size,
    ...
    if (KN_NODE_PROXY == node_type) {
        ...
        UCPCHECK_GOTO(ucc_tl_ucp_recv_nb(task->reduce_scatter_kn.proxy_buf[0],
                                         count * dt_size, mem_type, peer, team,
                                         task),
                      task, out);
    }
...
UCC_KN_PHASE_EXTRA:
    if ((KN_NODE_PROXY == node_type) || (KN_NODE_EXTRA == node_type)) {
        ...
        if (KN_NODE_EXTRA == node_type) {
            ...
            if (UCC_OK != (status = ucc_dt_reduce(
                               data_buf, task->reduce_scatter_kn.proxy_buf[0],
                               task->reduce_scatter_kn.proxy_buf[1], count, dt,
                               mem_type, args))) {
    ...
    if (args->coll_type == UCC_COLL_TYPE_ALLREDUCE &&
        ...
        ucc_kn_rsx_pattern_dst(size, rank, radix, count, &local_seg_offset,
                               &local_seg_count);
    }
    ...
    if (KN_NODE_PROXY == node_type &&
        ...
        UCPCHECK_GOTO(ucc_tl_ucp_send_nb(
                          PTR_OFFSET(task->reduce_scatter_kn.loop_sbuf[1],
...
static inline void setup_work_buffers_allreduce(ucc_tl_ucp_task_t *task)
{
    ...
    ucc_coll_args_t *  args = &TASK_ARGS(task);
    int                no_scratch =
        UCC_TL_UCP_TEAM_LIB(team)->cfg.allreduce_sra_kn_no_scratch &&
        !UCC_IS_INPLACE(*args) &&
...
static inline void setup_work_buffers_reduce_scatter(ucc_tl_ucp_task_t *task)
{
    ...
    if (args->coll_type == UCC_COLL_TYPE_REDUCE_SCATTER) {
        data_buf = UCC_IS_INPLACE(*args) ? args->dst.info.buffer
                                         : args->src.info.buffer;
        dest     = args->dst.info.buffer;
    } else {
        ucc_assert(args->coll_type == UCC_COLL_TYPE_REDUCE_SCATTERV);
        data_buf = UCC_IS_INPLACE(*args) ? args->dst.info_v.buffer
    ...
    if (KN_NODE_PROXY == task->reduce_scatter_kn.p.node_type) {
        if (UCC_IS_INPLACE(*args)) {
            ...
            task->reduce_scatter_kn.loop_sbuf[1] =
                task->reduce_scatter_kn.scratch_mc_header->addr;
            task->reduce_scatter_kn.loop_rbuf[0] =
                PTR_OFFSET(task->reduce_scatter_kn.scratch_mc_header->addr,
                           max_seg * dt_size);
            task->reduce_scatter_kn.loop_rbuf[1] =
                PTR_OFFSET(task->reduce_scatter_kn.scratch_mc_header->addr,
                           max_seg * dt_size);
            ...
            task->reduce_scatter_kn.proxy_buf[1] =
                task->reduce_scatter_kn.proxy_buf[0];

            task->reduce_scatter_kn.loop_sbuf[0] =
                task->reduce_scatter_kn.proxy_buf[0];
        ...
        task->reduce_scatter_kn.loop_sbuf[1] =
            task->reduce_scatter_kn.scratch_mc_header->addr;
        task->reduce_scatter_kn.loop_rbuf[0] = PTR_OFFSET(
            task->reduce_scatter_kn.scratch_mc_header->addr, max_seg * dt_size);
        task->reduce_scatter_kn.loop_rbuf[1] = PTR_OFFSET(
            task->reduce_scatter_kn.scratch_mc_header->addr, max_seg * dt_size);
    }
}
...
ucc_status_t ucc_tl_ucp_reduce_scatter_knomial_start(ucc_coll_task_t *coll_task)
{
    ...
    if (args->coll_type == UCC_COLL_TYPE_REDUCE_SCATTERV) {
        ucc_kn_rsv_pattern_init(size, rank, task->reduce_scatter_kn.p.radix,
                                args->dst.info_v.counts, UCC_COLL_ARGS64(args),
