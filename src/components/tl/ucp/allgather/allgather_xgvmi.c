...
ucc_status_t
ucc_tl_ucp_allgather_xgvmi_task_init(ucc_base_coll_args_t *coll_args,
                                              ...
                                              ucc_tl_ucp_task_t    *task)
{
    ...
    task->allgather_xgvmi.bufs = ptr;

    ptr = allgather_data = PTR_OFFSET(ptr, bufs_sz);
    task->allgather_xgvmi.allgather_data = allgather_data;

    gwbi_p = coll_args->args.global_work_buffer;
    task->super.bargs.args.global_work_buffer = gwbi_p;

    ptr = task->allgather_xgvmi.bufs->rbufs = PTR_OFFSET(ptr, allgather_data_sz);
    ptr = task->allgather_xgvmi.bufs->dst_rkeys = PTR_OFFSET(ptr, rbufs_sz);
    for (i = 0; i < team_size; i++) {
        task->allgather_xgvmi.bufs->dst_rkeys[i] = NULL;
    }

    ptr = task->allgather_xgvmi.bufs->dst_ebuf = PTR_OFFSET(ptr, dst_rkeys_sz);
    task->allgather_xgvmi.bufs->dst_ebuf->memh = NULL;

    allgather_data->dst_buf = dst_buf;

    task->allgather_xgvmi.allgather_data = allgather_data;
    task->allgather_xgvmi.allgather_task = NULL;

    if (!inplace) {
        allgather_data->src_buf = src_buf;

        ptr = task->allgather_xgvmi.bufs->sbufs = PTR_OFFSET(ptr, dst_ebuf_sz);
        ptr = task->allgather_xgvmi.bufs->src_rkeys = PTR_OFFSET(ptr, sbufs_sz);
        for (i = 0; i < team_size; i++) {
            task->allgather_xgvmi.bufs->src_rkeys[i] = NULL;
        }

        task->allgather_xgvmi.bufs->src_ebuf = PTR_OFFSET(ptr, src_rkeys_sz);
        task->allgather_xgvmi.bufs->src_ebuf->memh = NULL;
    } else {
        task->allgather_xgvmi.bufs->src_ebuf = NULL;
    }

    return UCC_OK;
}

ucc_status_t ucc_tl_ucp_allgather_xgvmi_allgather_info_finalize(
                ucc_tl_ucp_task_t *sw_task)
{
    ...
    ucc_rank_t         team_size  = UCC_TL_TEAM_SIZE(tl_team);
    void              *recvbuf    = sw_task->allgather_xgvmi.
    ...
    for (i = 0; i < team_size; i++) {
        ...
        sw_task->allgather_xgvmi.bufs->rbufs[i] =
            all_host_allgather[i].dst_buf;
        sw_task->allgather_xgvmi.bufs->dst_rkeys[i] = dst_unpacked;
        ...
        if (!inplace) {
            ...
            sw_task->allgather_xgvmi.bufs->sbufs[i] =
                all_host_allgather[i].src_buf;
            sw_task->allgather_xgvmi.bufs->src_rkeys[i] = src_unpacked;
        } else {
            sw_task->allgather_xgvmi.bufs->sbufs =
                sw_task->allgather_xgvmi.bufs->rbufs;
            sw_task->allgather_xgvmi.bufs->src_rkeys =
                sw_task->allgather_xgvmi.bufs->dst_rkeys;
        }
    }
...
void
ucc_tl_ucp_allgather_xgvmi_free_task(ucc_coll_task_t *coll_task)
{
    ...
    if (task->allgather_xgvmi.bufs) {
        if (!inplace) {
            if (task->allgather_xgvmi.bufs->src_ebuf->memh != NULL) {
                ucp_mem_unmap(tl_ctx->worker.ucp_context,
                              task->allgather_xgvmi.bufs->src_ebuf->memh);
                task->allgather_xgvmi.bufs->src_ebuf->memh = NULL;
            }
        }

        if (task->allgather_xgvmi.bufs->dst_ebuf->memh != NULL) {
            ucp_mem_unmap(tl_ctx->worker.ucp_context,
                          task->allgather_xgvmi.bufs->dst_ebuf->memh);
        }
        ucc_free(task->allgather_xgvmi.bufs);
    }
}

ucc_status_t
ucc_tl_ucp_allgather_xgvmi_start(ucc_coll_task_t *coll_task)
{
    ...
    allgather_data = rdma_task->allgather_xgvmi.allgather_data;

    rdma_task->allgather_xgvmi.gets_posted = 0;
    rdma_task->allgather_xgvmi.gets_completed = 0;
    memset(rdma_task->allgather_xgvmi.requests, 0,
    ...
    if (!inplace) {
        status = ucc_tl_ucp_allreduce_sliding_window_register(
            ...
            rdma_task->allgather_xgvmi.bufs->src_ebuf,
        ...
        ucc_assert(
            rdma_task->allgather_xgvmi.bufs->src_ebuf->packed_key_len
            <= ALLREDUCE_PACKED_KEY_MAX_LEN);
        memcpy(allgather_data->packed_src_key,
               rdma_task->allgather_xgvmi.bufs->src_ebuf->packed_key,
               rdma_task->allgather_xgvmi.bufs->src_ebuf->packed_key_len);
    }
    ...
    status = ucc_tl_ucp_allreduce_sliding_window_register(
        tl_ctx->worker.ucp_context, team,
        rdma_task->allgather_xgvmi.bufs->dst_ebuf,
    ...
    ucc_assert(
        rdma_task->allgather_xgvmi.bufs->dst_ebuf->packed_key_len
        <= ALLREDUCE_PACKED_KEY_MAX_LEN);
    memcpy(allgather_data->packed_dst_key,
           rdma_task->allgather_xgvmi.bufs->dst_ebuf->packed_key,
           rdma_task->allgather_xgvmi.bufs->dst_ebuf->packed_key_len);

    UCC_CHECK_GOTO(ucc_tl_ucp_allgather_ring_start(
                    rdma_task->allgather_xgvmi.allgather_task),
        out, status);
...
ucc_status_t
ucc_tl_ucp_allgather_xgvmi_finalize(ucc_coll_task_t *coll_task)
{
...
ucc_status_t
ucc_tl_ucp_allgather_xgvmi_rdma_task_post(
...
static inline void ucc_tl_ucp_allgather_xgvmi_free_rkeys(
    ucc_coll_task_t *coll_task)
{
    ucc_base_team_t   *team      = coll_task->team;
    ...
    for (i = 0; i < team_size; i++) {
        if (!inplace && task->allgather_xgvmi.bufs->src_rkeys[i] != NULL) {
            ucp_rkey_destroy(task->allgather_xgvmi.bufs->src_rkeys[i]);
        }
        if (task->allgather_xgvmi.bufs->dst_rkeys[i] != NULL) {
            ucp_rkey_destroy(task->allgather_xgvmi.bufs->dst_rkeys[i]);
        }
    }
}

static ucc_status_t
ucc_tl_ucp_allgather_xgvmi_rdma_task_finalize(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_status_t       st   = UCC_OK;

    ucc_tl_ucp_allgather_xgvmi_free_rkeys(coll_task);
    ucc_tl_ucp_allgather_xgvmi_free_task(coll_task);
...
ucc_tl_ucp_allgather_xgvmi_req_test(ucs_status_ptr_t   request,
                                             ucc_tl_ucp_task_t *task)
{
    if (request == NULL) {
        return UCC_OK;
...
static inline void ucc_tl_ucp_allgather_xgvmi_key_exchange_progress(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task      = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_coll_task_t   *allgather_task =
                           task->allgather_xgvmi.allgather_task;
    ...
    UCC_CHECK_GOTO(
        ucc_tl_ucp_allgather_xgvmi_allgather_info_finalize(task),
        err, status);
...
void ucc_tl_ucp_allgather_xgvmi_rdma_progress(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t    *task           = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ...
    ucc_tl_ucp_team_t    *tl_team        = ucc_derived_of(base_team, ucc_tl_ucp_team_t);
    ucc_coll_task_t      *allgather_task = task->allgather_xgvmi.allgather_task;
    ucc_tl_ucp_context_t *tl_ctx         = UCC_TL_UCP_TEAM_CTX(tl_team);
    ucp_request_param_t   req_param      = {0};
    int                   i              = 0;
    ucc_rank_t            rank           = UCC_TL_TEAM_RANK(tl_team);
    size_t                data_size      = (count * dt_size);
    ucs_status_ptr_t     *requests       = task->allgather_xgvmi.requests;
    int                  *posted         = &task->allgather_xgvmi.gets_posted;
    int                  *completed      = &task->allgather_xgvmi.gets_completed;
    void                 *src_addr;
    void                 *dst_addr;
    ucp_rkey_h            rkey;
    ucp_ep_h              ep;
    ...
    if (allgather_task != NULL) {
        ucc_tl_ucp_allgather_xgvmi_key_exchange_progress(coll_task);
        return;
    }
    ...
    for (i = *posted; i < host_team_size; i++) {
        ...
        req_param.memh = task->allgather_xgvmi.bufs->dst_ebuf->memh;
        src_addr = task->allgather_xgvmi.bufs->sbufs[offset];
        dst_addr = PTR_OFFSET(task->allgather_xgvmi.bufs->rbufs[rank], offset * data_size);
        rkey = task->allgather_xgvmi.bufs->src_rkeys[offset];
    ...
    for (i = *completed; i < *posted; i++) {
        if (ucc_tl_ucp_allgather_xgvmi_req_test(requests[i], task) == UCC_OK) {
...
ucc_status_t
ucc_tl_ucp_allgather_xgvmi_init(ucc_base_coll_args_t *coll_args,
                                ucc_base_team_t      *team,
                                ucc_coll_task_t     **task_h)
{
    ...
    *task_h                  = &schedule->super;
    schedule->super.post     = ucc_tl_ucp_allgather_xgvmi_start;
    schedule->super.progress = NULL;
    schedule->super.finalize = ucc_tl_ucp_allgather_xgvmi_finalize;
    ...
    status = ucc_tl_ucp_allgather_xgvmi_task_init(coll_args, team,
                                                           rdma_task);
    if (status != UCC_OK) {
    ...
    rdma_task->super.post     = ucc_tl_ucp_allgather_xgvmi_rdma_task_post;
    rdma_task->super.progress = ucc_tl_ucp_allgather_xgvmi_rdma_progress;
    rdma_task->super.finalize = ucc_tl_ucp_allgather_xgvmi_rdma_task_finalize;

    rdma_task->allgather_xgvmi.requests = ucc_malloc(sizeof(ucs_status_ptr_t) * size);
...
free_allgather_task:
    ucc_tl_ucp_coll_finalize(rdma_task->allgather_xgvmi.allgather_task);
free_rdma_task:
    ucc_tl_ucp_allgather_xgvmi_free_task(&rdma_task->super);
out:
    ucc_tl_ucp_put_schedule(schedule);
    return status;
}
