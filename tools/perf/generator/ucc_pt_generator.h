...
public:
    ...
    virtual size_t get_count_max() = 0; // max (src_count, dst_count) across all ranks (for a given iteration- max row or max column for a given matrix)
    ...
    virtual size_t get_dst_count_max() = 0; // max dst buffer count across iterations (sum row #rank for all matrices in file)
...
class ucc_pt_generator_matrix : public ucc_pt_generator_base
{
private:
    uint32_t comm_size;
    uint32_t rank_id;
