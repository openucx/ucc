...
public:
    ...
    void data_init(std::vector<op_t> &ops, UccTeam_h team) {
        ...
        for (int i = 0; i < ops.size(); i++) {
            ...
            uint64_t root = (aset_start + aset_stride*aset_root) % tsize;
            ...
            EXPECT_NE(aset.find(root), aset.end());
    ...
    void data_fini(UccTeam_h team)
    {
        ...
        for (int i = 0; i < ctxs.size(); i++) {
            for (int j = 0; j < ctxs[i].size(); j++ ) {
                ...
                uint64_t aset_start = coll->active_set.start;
                int64_t aset_stride = coll->active_set.stride;
                uint64_t root = (aset_start + aset_stride*coll->root) % tsize;
    ...
    {
        bool     ret  = true;
        int      root = 0, aset_root = 0;
        ucc_memory_type_t mem_type;
        uint8_t *rst;
        uint64_t aset_start, aset_stride;
        ucc_rank_t tsize = team->procs.size();
        ...
        for (int i = 0; i < ctx.size(); i++) {
            ...
            aset_root = ctx[i]->args->root;
            aset_start = ctx[i]->args->active_set.start;
            aset_stride = ctx[i]->args->active_set.stride;
            root = (aset_start + aset_stride*aset_root) % tsize;
        ...
        (
            ...
            (
                ...
                ({
                    ...
                    // root is in terms of the active_set
                    OP_T(0, 0, 1, 16, 8, HOST), // subset == full set
                    OP_T(0, 0, 2, 8, 6, HOST), // even ranks in full set
                    OP_T(0, 3, 8, 2, 65530, HOST), // pt2pt
                    OP_T(0, 1, 1, 2, 65531, HOST), // pt2pt
                    OP_T(0, 0, 1, 2, 8, HOST), // pt2pt
                    OP_T(0, 3, 4, 2, 1024, HOST), // pt2pt
                    OP_T(1, 3, 4, 2, 1023, HOST), // pt2pt
                    OP_T(0, 11, -8, 2, 65530, HOST), // pt2pt
                    OP_T(1, 11, -8, 2, 65531, HOST), // pt2pt
                    OP_T(0, 7, -2, 2, 123456, HOST), // pt2pt
                    OP_T(1, 7, -2, 2, 123455, HOST), // pt2pt
                    OP_T(0, 0, 1, 4, 1337, HOST),
                    OP_T(1, 0, 2, 4, 64, HOST),
                    OP_T(2, 0, 3, 3, 1335, HOST),
