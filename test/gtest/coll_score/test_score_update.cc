...
UCC_TEST_F(test_score_update, non_overlap)
{
    ...
    init_score(update, RLIST({RANGE(10, 20, 100), RANGE(30, 35, 1)}),
               UCC_COLL_TYPE_BARRIER);
    EXPECT_EQ(UCC_OK, ucc_coll_score_update(score, update));
    EXPECT_EQ(UCC_OK,
...
{
    init_score(score, RLIST({RANGE(0, 100, 100)}), UCC_COLL_TYPE_BARRIER, 0x1, 0x1);
    init_score(update, RLIST({RANGE(300, 400, 100)}), UCC_COLL_TYPE_BARRIER, 0x2, 0x2);
    EXPECT_EQ(UCC_OK, ucc_coll_score_update(score, update));
    EXPECT_EQ(UCC_OK,
              check_range(score, UCC_COLL_TYPE_BARRIER, UCC_MEMORY_TYPE_HOST,
...
{
    init_score(score, RLIST({RANGE(0, 100, 100)}), UCC_COLL_TYPE_BARRIER, 0x1, 0x1);
    init_score(update, RLIST({RANGE(0, 100, 100)}), UCC_COLL_TYPE_BARRIER, 0x2, 0x2);
    EXPECT_EQ(UCC_OK, ucc_coll_score_update(score, update));
    EXPECT_EQ(UCC_OK,
              check_range(score, UCC_COLL_TYPE_BARRIER, UCC_MEMORY_TYPE_HOST,
                          RLIST({RANGE(0, 100, 100)})));
    ...
    reset();
    init_score(score, RLIST({RANGE(0, 100, 100)}), UCC_COLL_TYPE_BARRIER, 0x1, 0x1);
    init_score(update, RLIST({RANGE(50, 150, 50)}), UCC_COLL_TYPE_BARRIER, 0x2, 0x2);
    EXPECT_EQ(UCC_OK, ucc_coll_score_update(score, update));
    EXPECT_EQ(UCC_OK,
              check_range(score, UCC_COLL_TYPE_BARRIER, UCC_MEMORY_TYPE_HOST,
    ...
    reset();
    init_score(score, RLIST({RANGE(50, 150, 100)}), UCC_COLL_TYPE_BARRIER, 0x1, 0x1);
    init_score(update, RLIST({RANGE(0, 100, 50)}), UCC_COLL_TYPE_BARRIER, 0x2, 0x2);
    EXPECT_EQ(UCC_OK, ucc_coll_score_update(score, update));
    EXPECT_EQ(UCC_OK,
              check_range(score, UCC_COLL_TYPE_BARRIER, UCC_MEMORY_TYPE_HOST,
