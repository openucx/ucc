...
class test_watchdog : public ucc::test {
public:
    void reset_to_default() {
        ucc::watchdog_signal();
        // all have to be set to their default values
        EXPECT_EQ(ucc::WATCHDOG_RUN, ucc::watchdog_get_state());
        EXPECT_EQ(ucc::watchdog_timeout_default, ucc::watchdog_get_timeout());
    }
};
...
UCS_TEST_F(test_watchdog, watchdog_set) {
    EXPECT_EQ(ucc::WATCHDOG_RUN, ucc::watchdog_get_state());
    EXPECT_EQ(ucc::watchdog_timeout_default, ucc::watchdog_get_timeout());
    EXPECT_EQ(SIGABRT, ucc::watchdog_get_kill_signal());

    ucc::watchdog_set(ucc::WATCHDOG_TEST);
    // when the test state is applied, the watchdog
    // changes state to WATCHDOG_DEFAULT_SET
    EXPECT_EQ(ucc::WATCHDOG_DEFAULT_SET, ucc::watchdog_get_state());
    EXPECT_EQ(ucc::watchdog_timeout_default, ucc::watchdog_get_timeout());
    EXPECT_EQ(SIGTERM, ucc::watchdog_get_kill_signal());

    reset_to_default();

    ucc::watchdog_set(500.);
    EXPECT_EQ(ucc::WATCHDOG_DEFAULT_SET, ucc::watchdog_get_state());
    EXPECT_EQ(500., ucc::watchdog_get_timeout());
    EXPECT_EQ(SIGABRT, ucc::watchdog_get_kill_signal());

    reset_to_default();

    ucc::watchdog_set(ucc::WATCHDOG_TEST, 100.);
    // when the test state and the timeout are applied,
    // the watchdog changes state to WATCHDOG_DEFAULT_SET
    EXPECT_EQ(ucc::WATCHDOG_DEFAULT_SET, ucc::watchdog_get_state());
    EXPECT_EQ(100., ucc::watchdog_get_timeout());
    EXPECT_EQ(SIGTERM, ucc::watchdog_get_kill_signal());

    reset_to_default();

    ucc::watchdog_set(ucc::WATCHDOG_DEFAULT_SET, 200.);
    ...
    ucc::watchdog_set(ucc::WATCHDOG_DEFAULT_SET);
    EXPECT_EQ(ucc::WATCHDOG_RUN, ucc::watchdog_get_state());
    EXPECT_EQ(ucc::watchdog_timeout_default, ucc::watchdog_get_timeout());
    EXPECT_EQ(SIGABRT, ucc::watchdog_get_kill_signal());
}
...
UCS_TEST_F(test_watchdog, watchdog_timeout) {
    ...
    ucc::watchdog_set(ucc::WATCHDOG_TEST, timeout);
