...
if [ ! -x "${MPIRUN}" ]; then
    echo "CACHE_EQUIV: ERROR - mpirun not found; set MPIRUN" >&2
    exit 1
fi
...
# "equivalent" by never touching the cache at all. The correctness suite is
# tallied into the ucc_test_mpi report and exit code, so a failure is caught
# below.
#
# UCC_TEAM_CACHE_MAX_SIZE=2 keeps the cache small enough that the overlapping
# subcommunicator tests actually evict and diverge; without it those tests skip.
run_mpi_pass() {
...
echo ""
echo "--- Pass A: UCC_TEAM_CACHE_ENABLE=y ---"
pass_a_rc=0
run_mpi_pass "CACHE_ON" "y" "${TEAMS}" || pass_a_rc=$?
...
echo ""
echo "--- Pass B: UCC_TEAM_CACHE_ENABLE=n ---"
pass_b_rc=0
run_mpi_pass "CACHE_OFF" "n" "${TEAMS}" || pass_b_rc=$?
