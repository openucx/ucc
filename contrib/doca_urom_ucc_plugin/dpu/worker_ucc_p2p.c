...
void urom_ep_err_cb(void *arg, ucp_ep_h ep, ucs_status_t ucs_status)
{
	...
	DOCA_LOG_ERR("Endpoint error detected, status: %s", ucs_status_string(ucs_status));
}
 ...
 */
static doca_error_t worker_ucc_ep_lookup(struct urom_worker_ucc *ucc_worker, uint64_t dest, ucp_ep_h *ep)
{
	int ret;
	khint_t k;
	void *addr;
	...
	/* Create new EP */
	status = doca_urom_worker_domain_addr_lookup(ucc_worker->super, dest, &addr);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t worker_ucc_memh_lookup(struct urom_worker_ucc *ucc_worker, uint64_t dest, ucp_mem_h *memh)
{
	int ret;
	khint_t k;
	void *mem_handle;
	...
	status = doca_urom_worker_domain_memh_lookup(ucc_worker->super, dest, 0, &memh_len, &mem_handle);
	if (status != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Id not found in domain:: %#lx", dest);
		return DOCA_ERROR_NOT_FOUND;
	}
	...
	ucs_status = ucp_mem_map(ucc_worker->ucp_data.ucp_context, &mmap_params, &memh_id);
	if (ucs_status != UCS_OK) {
					  ...
					  void **ret_rkey)
{
	...
	status = doca_urom_worker_domain_seg_lookup(ucc_worker->super, dest, va, &seg);
	if (status != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Id not found in domain: %#lx", dest);
		return DOCA_ERROR_NOT_FOUND;
	}

	status = doca_urom_worker_domain_mkey_lookup(ucc_worker->super, dest, seg, &packed_key_len, &packed_key);
	if (status != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Id not found in domain: %#lx", dest);
		return DOCA_ERROR_NOT_FOUND;
	}
 ...
 */
static void send_completion_cb(void *request, ucs_status_t status, void *user_data)
{
 ...
 * @user_data [in]: UCC data
 */
static void recv_completion_cb(void *request, ucs_status_t status, const ucp_tag_recv_info_t *info, void *user_data)
{
	(void)info;
	int *req = (int *)user_data;
			 ...
			 int *req)
{
	...
	*req = 0;
	req_param.op_attr_mask = UCP_OP_ATTR_FIELD_DATATYPE | UCP_OP_ATTR_FIELD_CALLBACK | UCP_OP_ATTR_FIELD_USER_DATA;
...
doca_error_t ucc_recv_nb(void *msg, size_t len, int64_t dest, struct urom_worker_ucc *ucc_worker, int *req)
{
	...
	*req = 0;
	...
	/* Process tag recv */
	ucp_status = ucp_tag_recv_nbx(ucc_worker->ucp_data.ucp_worker, msg, 1, dest, 0xffff, &req_param);
	if (ucp_status != UCS_OK) {
		if (UCS_PTR_IS_ERR(ucp_status)) {
			ucp_request_cancel(ucc_worker->ucp_data.ucp_worker, ucp_status);
			ucp_request_free(ucp_status);
			 ...
			 struct urom_worker_ucc *ucc_worker)
{
	...
	if (dest == MAX_HOST_DEST_ID)
		...
		urom_status = worker_ucc_ep_lookup(ucc_worker, dest, &ep);
		if (urom_status != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to find peer %ld to complete collective", dest);
			return DOCA_ERROR_NOT_FOUND;
		}
	}
	...
	if (dest == MAX_HOST_DEST_ID) {
		urom_status = worker_ucc_key_lookup(ucc_worker, myrank, ep, rva, (void **)&rkey);
		if (urom_status != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to lookup rkey for peer %ld", dest);
			return DOCA_ERROR_NOT_FOUND;
		}
	} else {
		urom_status = worker_ucc_key_lookup(ucc_worker, dest, ep, rva, (void **)&rkey);
		if (urom_status != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to lookup rkey for peer %ld", dest);
			return DOCA_ERROR_NOT_FOUND;
		}
