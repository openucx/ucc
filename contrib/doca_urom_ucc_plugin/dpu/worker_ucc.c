...
static int ucc_component_enabled;	  /* Shared variable between UCC worker threads */
 ...
 */
static doca_error_t find_qe_slot(uint64_t ctx_id, struct urom_worker_ucc *ucc_worker, struct ucc_queue_element **ret_qe)
{
...
static doca_error_t urom_worker_ucc_open(struct urom_worker_ctx *ctx)
{
	...
	if (worker_ucc_opts.num_progress_threads < MIN_THREADS) {
		...
		DOCA_LOG_WARN("Number of threads for UCC Offload must be 1 or more, set to 1");
	}

	ucc_worker->ctx_id = 0;
	ucc_worker->nr_connections = 0;
	ucc_worker->ucc_data = calloc(worker_ucc_opts.ppw * worker_ucc_opts.tpp, sizeof(struct ucc_data));
	...
	ucc_worker->queue = (struct ucc_queue_element **)malloc(sizeof(struct ucc_queue_element *) *
	...
	for (i = 0; i < worker_ucc_opts.num_progress_threads; i++) {
		ucc_worker->queue[i] = calloc(worker_ucc_opts.list_size, sizeof(struct ucc_queue_element));
	...
	queue_front = (volatile uint64_t *)calloc(worker_ucc_opts.num_progress_threads, sizeof(uint64_t));
	if (queue_front == NULL) {
		result = DOCA_ERROR_NO_MEMORY;
		goto queue_free;
	}

	queue_tail = (volatile uint64_t *)calloc(worker_ucc_opts.num_progress_threads, sizeof(uint64_t));
	if (queue_tail == NULL) {
		result = DOCA_ERROR_NO_MEMORY;
		goto queue_front_free;
	}

	queue_size = (volatile uint64_t *)calloc(worker_ucc_opts.num_progress_threads, sizeof(uint64_t));
	if (queue_size == NULL) {
		result = DOCA_ERROR_NO_MEMORY;
	...
	status = ucp_init(&ucp_params, ucp_config, &ucc_worker->ucp_data.ucp_context);
	ucp_config_release(ucp_config);
	if (status != UCS_OK) {
	...
	status = ucp_worker_create(ucc_worker->ucp_data.ucp_context, &worker_params, &ucc_worker->ucp_data.ucp_worker);
	if (status != UCS_OK) {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_cmd_unpack(void *packed_cmd, size_t packed_cmd_len, struct urom_worker_cmd **cmd)
{
	void *ptr;
	...
	size_t team_size, count_pack_size, disp_pack_size;
	...
	*cmd = packed_cmd;
	ptr = packed_cmd + ucs_offsetof(struct urom_worker_cmd, plugin_cmd) + sizeof(struct urom_worker_ucc_cmd);
	...
	case UROM_WORKER_CMD_UCC_COLL:
		...
		if (coll_args->coll_type == UCC_COLL_TYPE_ALLTOALLV ||
		    coll_args->coll_type == UCC_COLL_TYPE_ALLGATHERV || coll_args->coll_type == UCC_COLL_TYPE_GATHERV ||
			...
			count_pack_size = ((is_count_64) ? sizeof(uint64_t) : sizeof(uint32_t)) * team_size;
			disp_pack_size = ((is_disp_64) ? sizeof(uint64_t) : sizeof(uint32_t)) * team_size;
 ...
 */
static void ucc_worker_safe_push_notification(struct urom_worker_ucc *ucc_worker, struct urom_worker_notif_desc *nd)
{
	uint64_t lvalue = 0;
 ...
 */
static void worker_ucc_dest_remove(struct urom_worker_ucc *ucc_worker, uint64_t dest_id)
{
	khint_t k;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t worker_ucc_dest_lookup(struct urom_worker_ucc *ucc_worker, uint64_t dest_id, uint64_t *ctx_id)
{
	int ret;
	khint_t k;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_lib_create(struct urom_worker_ucc *ucc_worker,
					       struct urom_worker_cmd_desc *cmd_desc)
{
	...
	for (i = 0; i < worker_ucc_opts.tpp; i++) {
		ucc_status = ucc_init(lib_params, lib_config, &ucc_worker->ucc_data[ctx_id*worker_ucc_opts.tpp + i].ucc_lib);
...
static doca_error_t ucc_worker_lib_destroy(struct urom_worker_ucc *ucc_worker)
{
	...
	for (j = 0; j < ucc_worker->nr_connections; j++) {
		for (k = 0; k < worker_ucc_opts.tpp; k++) {
			for (i = 0; i < ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].n_teams; i++) {
				if (!ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_team[i])
					continue;
				status = ucc_team_destroy(ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_team[i]);
				if (status != UCC_OK) {
					DOCA_LOG_ERR("Failed to destroy UCC team of data index %lu and team index %ld", j, i);
					result = DOCA_ERROR_DRIVER;
				}
				free(ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].pSync);
			}
			if (ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_context) {
				status = ucc_context_destroy(ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_context);
				if (status != UCC_OK) {
					DOCA_LOG_ERR("Failed to destroy UCC context of UCC data index %lu", j);
					result = DOCA_ERROR_DRIVER;
				}
				ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_context = NULL;
			}
			if (ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_lib) {
				status = ucc_finalize(ucc_worker->ucc_data[j*worker_ucc_opts.tpp + k].ucc_lib);
				if (status != UCC_OK) {
					DOCA_LOG_ERR("Failed to finalize UCC lib of UCC data index %lu", j);
					result = DOCA_ERROR_DRIVER;
				}
			}
		}
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_lib_destroy(struct urom_worker_ucc *ucc_worker,
						struct urom_worker_cmd_desc *cmd_desc)
{
	struct urom_worker_notify *notif;
	struct urom_worker_notif_desc *nd;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t handle_progress_thread_coll_element(struct ucc_queue_element *qe,
							struct urom_worker_ucc *ucc_worker,
							int thread_id)
{
	int64_t lvalue = 0;
	ucc_status_t tmp_status, ucc_status = UCC_OK;
	...
	if (!qe->posted) {
		...
		if (UCC_OK != ucc_status) {
			DOCA_LOG_ERR("Failed to post UCC collective: %s", ucc_status_string(ucc_status));
	...
	if (ucc_status == UCC_INPROGRESS) {
		...
		*qe_back = *qe;
		qe->in_use = 0;
		queue_front[thread_id] = (queue_front[thread_id] + 1) % worker_ucc_opts.list_size;
		...
		if (qe->old_dest) {
			DOCA_LOG_DBG("Putting data back to host %p with size %lu", qe->old_dest, qe->data_size);
			if (qe->dest_packed_key != NULL) {
				status = ucc_rma_put_host(ucc_worker->ucc_data[qe->ctx_id].local_work_buffer +
								  qe->data_size,
							  qe->old_dest,
							  qe->data_size,
							  qe->ctx_id,
				...
				status = ucc_rma_put(ucc_worker->ucc_data[qe->ctx_id].local_work_buffer + qe->data_size,
						     qe->old_dest,
						     qe->data_size,
						     MAX_HOST_DEST_ID,
						     qe->myrank,
...
exit:
	if (qe->nd != NULL) {
		ucc_notif = (struct urom_worker_notify_ucc *)qe->nd->worker_notif.plugin_notif;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t handle_progress_thread_team_element(struct ucc_queue_element *qe,
							struct urom_worker_ucc *ucc_worker,
							int thread_id)
{
	int64_t lvalue = 0;
	...
	ucc_status = ucc_team_create_test(ucc_worker->ucc_data[qe->ctx_id].ucc_team[qe->team_id]);
	if (ucc_status == UCC_INPROGRESS) {
		ucc_status = ucc_context_progress(ucc_worker->ucc_data[qe->ctx_id].ucc_context);
		lvalue = ucs_atomic_cswap64(&queue_lock, 0, 1);
		while (lvalue != 0)
			lvalue = ucs_atomic_cswap64(&queue_lock, 0, 1);
		status = find_qe_slot(qe->ctx_id, ucc_worker, &qe_back);
		...
		*qe_back = *qe;
		...
		DOCA_LOG_INFO("Finished team creation (%ld:%ld)", qe->ctx_id, qe->team_id);
		if (ucc_notif)
			ucc_notif->team_create_nqe.team = ucc_worker->ucc_data[qe->ctx_id].ucc_team[qe->team_id];
		status = DOCA_SUCCESS;
	}
...
static void *urom_worker_ucc_progress_thread(void *arg)
{
	...
	int i, front, size, thread_id = targs->thread_id;
	...
	while (ucc_component_enabled) {
		...
		for (i = 0; i < size; i++) {
			...
			if (qe->in_use != 1) {
				DOCA_LOG_WARN("Found queue element in queue and marked not in use");
				continue;
			}
			if (qe->type == UCC_WORKER_QUEUE_ELEMENT_TYPE_TEAM_CREATE) {
				status = handle_progress_thread_team_element(qe, ucc_worker, thread_id);
				...
				status = handle_progress_thread_coll_element(qe, ucc_worker, thread_id);
				if (status == DOCA_ERROR_IN_PROGRESS)
					continue;
 ...
 * @return: UCC_OK on success and UCC_ERR otherwise
 */
static ucc_status_t urom_worker_ucc_oob_allgather(void *sbuf, void *rbuf, size_t msglen, void *oob_coll_ctx, void **req)
{
	char *recv_buf;
	int index, size, i;
	...
	*req = oob_req;
	...
	for (i = 0; i < size; i++)
		ucc_send_nb(sbuf, msglen, index, i, ctx->ucc_worker, &oob_req->status[i + size]);
...
static ucc_status_t urom_worker_ucc_oob_allgather_test(void *req)
{
	struct coll_ctx *ctx;
	struct oob_allgather_req *oob_req;
	int i, probe_count, nr_done, size, nr_probes = 5;
	...
	for (probe_count = 0; probe_count < nr_probes; probe_count++) {
		nr_done = 0;
		for (i = 0; i < size * 2; i++) {
			if (oob_req->status[i] != 1 && ctx->ucc_worker->ucp_data.ucp_worker != NULL)
...
static void *urom_worker_ucc_ctx_progress_thread(void *arg)
{
	...
	maps = (ucc_mem_map_t **) calloc(worker_ucc_opts.tpp, sizeof(ucc_mem_map_t *));
	coll_ctx = (struct coll_ctx **) calloc(worker_ucc_opts.tpp, sizeof(struct coll_ctx *));

	for (i = 0; i < worker_ucc_opts.tpp; i++) {
		uint64_t thread_ctx_id = ctx_id*worker_ucc_opts.tpp + i;

		if (ucc_worker->ucc_data[thread_ctx_id].ucc_lib == NULL) {
			DOCA_LOG_ERR("Attempting to create UCC context without first initializing a UCC lib");
			status = DOCA_ERROR_BAD_STATE;
			goto fail;
		}

		if (ucc_context_config_read(ucc_worker->ucc_data[thread_ctx_id].ucc_lib, NULL, &ctx_config) != UCC_OK) {
		...
		ucc_status = ucc_context_config_modify(ctx_config, NULL, "ESTIMATED_NUM_EPS", str_buf);
		if (ucc_status != UCC_OK) {
			DOCA_LOG_ERR("UCC context config modify failed for estimated_num_eps");
			status = DOCA_ERROR_DRIVER;
			goto cfg_release;
		}

		ucc_worker->ucc_data[thread_ctx_id].local_work_buffer = calloc(1, len * 2);
		...
		ucc_worker->ucc_data[thread_ctx_id].pSync = calloc(worker_ucc_opts.num_psync, sizeof(long));
		...
		maps[i][0].address = ucc_worker->ucc_data[thread_ctx_id].local_work_buffer;
		maps[i][0].len = len * 2;
		...
		ucc_status = ucc_context_create(ucc_worker->ucc_data[thread_ctx_id].ucc_lib,
						&ctx_params,
						ctx_config,
	...
	status = DOCA_SUCCESS;
	ucc_notif->context_create_nqe.context = ucc_worker->ucc_data[ctx_id].ucc_context;
	DOCA_LOG_DBG("UCC context created, ctx_id %lu, context %p", ctx_id, ucc_worker->ucc_data[ctx_id].ucc_context);
	goto exit;
...
context_destroy:
	for(i = 0; i < worker_ucc_opts.tpp; i++) {
		if(ucc_worker->ucc_data[ctx_id*worker_ucc_opts.tpp + i].ucc_context) ucc_context_destroy(ucc_worker->ucc_data[ctx_id*worker_ucc_opts.tpp + i].ucc_context);
	}
coll_free:
	for(i = 0; i < worker_ucc_opts.tpp; i++) {
		if(coll_ctx[i]) free(coll_ctx[i]);
	}
	free(coll_ctx);
maps_free:
	for(i = 0; i < worker_ucc_opts.tpp; i++) {
...
buf_free:
	for(i = 0; i < worker_ucc_opts.tpp; i++) {
		if(ucc_worker->ucc_data[ctx_id*worker_ucc_opts.tpp + i].local_work_buffer)
			free(ucc_worker->ucc_data[ctx_id*worker_ucc_opts.tpp + i].local_work_buffer);
	}
cfg_release:
	ucc_context_config_release(ctx_config);
fail:
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_context_create(struct urom_worker_ucc *ucc_worker,
						   struct urom_worker_cmd_desc *cmd_desc)
{
	int ret;
	...
	ret = pthread_create(&context_progress_thread, NULL, urom_worker_ucc_ctx_progress_thread, (void *)args);
	if (ret != 0) {
		nd = calloc(1, sizeof(*nd) + sizeof(*ucc_notif));
		if (nd == NULL)
			return DOCA_ERROR_NO_MEMORY;
		...
		ucc_notif = (struct urom_worker_notify_ucc *)nd->worker_notif.plugin_notif;
		ucc_notif->dpu_worker_id = ucc_cmd->dpu_worker_id;
		ucc_notif->notify_type = UROM_WORKER_NOTIFY_UCC_CONTEXT_CREATE_COMPLETE;
		ucc_worker_safe_push_notification(ucc_worker, nd);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_context_destroy(struct urom_worker_ucc *ucc_worker,
						    struct urom_worker_cmd_desc *cmd_desc)
{
	uint64_t ctx_id, i;
	doca_error_t status;
	...
	for (i = 0; i < worker_ucc_opts.tpp; i++) {
		...
		if (ucc_worker->ucc_data[thread_ctx_id].ucc_context) {
			if (ucc_context_destroy(ucc_worker->ucc_data[thread_ctx_id].ucc_context) != UCC_OK) {
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_team_create(struct urom_worker_ucc *ucc_worker,
						struct urom_worker_cmd_desc *cmd_desc)
{
	uint64_t ctx_id, i;
	ucc_ep_map_t map;
	...
	for (i = 0; i < worker_ucc_opts.tpp; i++) {
		...
		if (ucc_worker->ucc_data[thread_ctx_id].ucc_context == NULL ||
			ucc_cmd->team_create_cmd.context_h != ucc_worker->ucc_data[ctx_id].ucc_context) {
			DOCA_LOG_ERR("Attempting to create UCC team over non-existent context");
		...
		team_params.mask = UCC_TEAM_PARAM_FIELD_EP | UCC_TEAM_PARAM_FIELD_TEAM_SIZE | UCC_TEAM_PARAM_FIELD_EP_MAP |
		...
		if (ucc_worker->ucc_data[thread_ctx_id].ucc_team == NULL) {
			ucc_worker->ucc_data[thread_ctx_id].ucc_team = malloc(sizeof(ucc_worker->ucc_data[thread_ctx_id].ucc_team));
			if (ucc_worker->ucc_data[thread_ctx_id].ucc_team == NULL) {
				status = DOCA_ERROR_NO_MEMORY;
				goto coll_free;
			}
		...
		ucc_status = ucc_team_create_post(&ucc_worker->ucc_data[thread_ctx_id].ucc_context,
						1,
						&team_params,
		...
		if (i == 0)
			...
			qe->nd = NULL;
		ucs_atomic_add64(&queue_size[thread_ctx_id % worker_ucc_opts.num_progress_threads], 1);
...
static doca_error_t post_nthreads_colls(
    uint64_t ctx_id, struct urom_worker_ucc *ucc_worker, ucc_coll_args_t *coll_args,
	...
	ucc_worker_key_buf * key_duplicate_per_rank)
{
    ...
    size_t src_thread_size =
        src_thread_count * urom_worker_get_dt_size(coll_args->src.info.datatype);
    size_t dst_thread_size =
        dst_thread_count * urom_worker_get_dt_size(coll_args->dst.info.datatype);
    ...
    for(i = 0; i < threads; i++) {
		...
		if (gwbi == NULL) {
			DOCA_LOG_ERR("Failed to initialize UCC collective: Couldnt malloc global work buffer");
			status = DOCA_ERROR_DRIVER;
			goto fail;
		}

		gwbi->packed_src_memh = key_duplicate_per_rank[i].rkeys;
		gwbi->packed_dst_memh = key_duplicate_per_rank[i].rkeys + key_duplicate_per_rank[i].src_len;

		coll_args->global_work_buffer = gwbi;

        if(!in_place) coll_args->src.info.count = src_thread_count;
        coll_args->dst.info.count = dst_thread_count;

        if(!in_place) coll_args->src.info.buffer = src_buf + i * src_thread_size;
        coll_args->dst.info.buffer = dst_buf + i * dst_thread_size;

        if (i == threads - 1) {
            if(!in_place) coll_args->src.info.count += src_count % threads;
		...
		ucc_status = ucc_collective_init(coll_args, &coll_req, ucc_worker->ucc_data[thread_ctx_id].ucc_team[team_idx]);
		if (UCC_OK != ucc_status) {
			DOCA_LOG_ERR("Failed to initialize UCC collective: %s", ucc_status_string(ucc_status));
			status = DOCA_ERROR_DRIVER;
			goto fail;
		}

		if (thread_ctx_id >= worker_ucc_opts.num_progress_threads) {
			DOCA_LOG_ERR("Warning--possible deadlock: multiple threads posting to the same queue, and the qe is going to barrier. Ensure tpp < num progress threads to avoid this\n");
		}
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_coll_init(struct urom_worker_ucc *ucc_worker, struct urom_worker_cmd_desc *cmd_desc)
{
	ucc_worker_key_buf *key_duplicate_per_rank;
	...
	if (ucc_cmd->coll_cmd.work_buffer_size > 0 && ucc_cmd->coll_cmd.work_buffer)
	...
	if (coll_args->coll_type == UCC_COLL_TYPE_ALLTOALL || coll_args->coll_type == UCC_COLL_TYPE_ALLTOALLV) {
		if (!ucc_cmd->coll_cmd.use_xgvmi) {
			...
			if (coll_args->mask & UCC_COLL_ARGS_FIELD_GLOBAL_WORK_BUFFER) {
				...
				keys = work_buffer;
				status = ucc_rma_get_host(ucc_worker->ucc_data[ctx_id].local_work_buffer,
				...
				status = ucc_rma_get(ucc_worker->ucc_data[ctx_id].local_work_buffer,
						     coll_args->src.info.buffer,
						     size,
						     MAX_HOST_DEST_ID,
						     myrank,
			...
			coll_args->src.info.buffer = ucc_worker->ucc_data[ctx_id].local_work_buffer;
			old_dest = coll_args->dst.info.buffer;
			coll_args->dst.info.buffer = ucc_worker->ucc_data[ctx_id].local_work_buffer + size;
		}
		if (!(coll_args->mask & UCC_COLL_ARGS_FIELD_GLOBAL_WORK_BUFFER) || !work_buffer) {
			coll_args->mask |= UCC_COLL_ARGS_FIELD_FLAGS | UCC_COLL_ARGS_FIELD_GLOBAL_WORK_BUFFER;
			coll_args->flags |= UCC_COLL_ARGS_FLAG_MEM_MAPPED_BUFFERS;
			coll_args->global_work_buffer =
				ucc_worker->ucc_data[ctx_id].pSync +
				(ucc_worker->ucc_data[ctx_id].psync_offset % worker_ucc_opts.num_psync);
			ucc_worker->ucc_data[ctx_id].psync_offset++;
		} else {
			if (work_buffer != NULL)
				coll_args->global_work_buffer = work_buffer;
		}
	} else if (coll_args->coll_type == UCC_COLL_TYPE_ALLREDUCE || coll_args->coll_type == UCC_COLL_TYPE_ALLGATHER) {
		if (!ucc_cmd->coll_cmd.use_xgvmi) {
			DOCA_LOG_ERR("Failed to initialize UCC collective: Allreduce must use xgvmi");
			status = DOCA_ERROR_DRIVER;
			goto fail;
		}
		if (!(coll_args->mask & UCC_COLL_ARGS_FIELD_GLOBAL_WORK_BUFFER) || !work_buffer) {
			DOCA_LOG_ERR("Failed to initialize UCC collective: Allreduce must use global work buffer");
			status = DOCA_ERROR_DRIVER;
			goto fail;
		}
		...
		key_duplicate_per_rank = malloc(sizeof(ucc_worker_key_buf) * worker_ucc_opts.tpp);
		if (key_duplicate_per_rank == NULL) printf("couldnt malloc key_duplicate_per_rank\n");
		for (i = 0; i < worker_ucc_opts.tpp; i++) {
			memcpy(key_duplicate_per_rank[i].rkeys, keys->rkeys, keys->src_len + keys->dst_len);
	...
	qe->barrier = NULL;
	qe->nd = nd;
	ucs_atomic_add64(&queue_size[ctx_id % worker_ucc_opts.num_progress_threads], 1);
	return DOCA_SUCCESS;
req_destroy:
	ucc_collective_finalize(coll_req);
fail:
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_pass_dc_create(struct urom_worker_ucc *ucc_worker,
						   struct urom_worker_cmd_desc *cmd_desc)
{
	uint64_t ctx_id;
	ucp_ep_h new_ep;
	...
	ucc_notif = (struct urom_worker_notify_ucc *)notif->plugin_notif;
	ucc_notif->notify_type = UROM_WORKER_NOTIFY_UCC_PASSIVE_DATA_CHANNEL_COMPLETE;
	ucc_notif->dpu_worker_id = ucc_cmd->dpu_worker_id;
	...
	if (ucc_worker->ucc_data[ctx_id].host == NULL) {
		...
		ucs_status = ucp_ep_create(ucc_worker->ucp_data.ucp_worker, &ep_params, &new_ep);
		...
		DOCA_LOG_DBG("Created passive data channel for host for rank %lu", ucc_cmd->dpu_worker_id);
	} else
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_worker_cmd(struct urom_worker_ctx *ctx, ucs_list_link_t *cmd_list)
{
	struct urom_worker_cmd *cmd;
	doca_error_t status = DOCA_SUCCESS;
	struct urom_worker_ucc_cmd *ucc_cmd;
	struct urom_worker_cmd_desc *cmd_desc;
	struct urom_worker_ucc *ucc_worker = (struct urom_worker_ucc *)ctx->plugin_ctx;

	while (!ucs_list_is_empty(cmd_list)) {
		cmd_desc = ucs_list_extract_head(cmd_list, struct urom_worker_cmd_desc, entry);
		status = urom_worker_ucc_cmd_unpack(&cmd_desc->worker_cmd, cmd_desc->worker_cmd.len, &cmd);
		if (status != DOCA_SUCCESS) {
			free(cmd_desc);
			return status;
		}
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_addr(struct urom_worker_ctx *worker_ctx, void *addr, uint64_t *addr_len)
{
	ucs_status_t status;
	struct urom_worker_ucc *ucc_worker = (struct urom_worker_ucc *)worker_ctx->plugin_ctx;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_progress(struct urom_worker_ctx *ctx, ucs_list_link_t *notif_list)
{
	uint64_t lvalue = 0;
	struct urom_worker_notif_desc *nd;
	struct urom_worker_ucc *ucc_worker = (struct urom_worker_ucc *)ctx->plugin_ctx;
	...
	while (!ucs_list_is_empty(&ucc_worker->completed_reqs)) {
		nd = ucs_list_extract_head(&ucc_worker->completed_reqs, struct urom_worker_notif_desc, entry);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t urom_worker_ucc_notif_pack(struct urom_worker_notify *notif,
					       ...
					       void *packed_notif)
{
	...
	/* Pack base command */
	pack_len = ucs_offsetof(struct urom_worker_notify, plugin_notif) + sizeof(struct urom_worker_notify_ucc);
